    ...
    if (isMainPage) { // 메인 페이지(index.html)에서만 실행될 로직
        ...
        function showQuestion() {
            if (currentQuestionIndex < questions.length) {
                ...
                questionCard.options = questionData.options;
        ...
        function handleAnswerClick(event) {
            const chosenType = event.detail.type;
            scores[chosenType] += 3;
        ...
        function shareResult() {
            const fullResultText = resultTitle.textContent;
            const resultDescText = resultDescription.textContent;
            const shareText = `${fullResultText} 나의 성격은? ${resultDescText.substring(0, 50)}... 지금 테스트해보세요!`;
